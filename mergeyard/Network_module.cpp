...
void network_init() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("Check SSID/PW and ensures 2.4GHz network.");
  }
}

void upload_integrated_data(float temp, float ec, uint8_t *imgBuf,
                            size_t imgLen) {
  Serial.println("[Network] upload_integrated_data called.");

  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("[Network] Abort: WiFi not connected.");
    return;
  }
  ...
  Serial.println("[Network] Payload created. Size: " +
                 String(jsonPayload.length()));
  Serial.println("[Network] Target: " + String(SERVER_POST_URL));
  Serial.println("[Network] Sending POST...");
  ...
  if (httpResponseCode > 0) {
    ...
    if (httpResponseCode != 200) {
      Serial.println("Warning: Server returned non-200 response.");
    }
  } else {
...
void poll_motor_command() {
  ...
  if (httpResponseCode == 200) {
    ...
    Serial.print("HTTP GET Error (Polling): ");
    Serial.println(http.errorToString(httpResponseCode));
    Serial.print("Target URL: ");
    Serial.println(SERVER_GET_URL);
  }
  http.end();
}
